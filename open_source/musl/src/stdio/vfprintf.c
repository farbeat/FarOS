...
#ifdef __XLT_OS__
static int vfprintf_core(FILE *restrict f, const char *restrict fmt, va_list ap, const struct lockfile_func *l)
#else
...
#ifdef __XLT_OS__
#ifdef CONFIG_LIBC_STDIO
int vfprintf(FILE *restrict f, const char *restrict fmt, va_list ap)
{
	struct lockfile_func l = {
		.lock = __lockfile,
		.unlock = __unlockfile,
	};
	return vfprintf_core(f, fmt, ap, &l);
}
#endif

int vfprintf_unlocked(FILE *restrict f, const char *restrict fmt, va_list ap)
{
	return vfprintf_core(f, fmt, ap, (struct lockfile_func *)0);
}
#endif