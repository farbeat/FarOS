    ...
    switch (_IOC_DIR(cmd)) {
        ...
        case (_IOC_WRITE | _IOC_READ):
            ...
            if (_IOC_DIR(cmd) & _IOC_WRITE) {
                if (osal_copy_from_user(*ioctl_arg, (void __user *)arg, arg_size) != 0) {
                    osal_log("osal_copy_from_user failed, cmd=0x%x\n", cmd);
                    return -1;
                }
            }
            break;
...
static int osal_ioctl_copy_to_user(unsigned int cmd, unsigned long arg, void *ioctl_arg, unsigned int arg_size)
{
    ...
    switch (_IOC_DIR(cmd)) {
        case _IOC_READ:
        case (_IOC_WRITE | _IOC_READ): {
                errno_t ret = memcpy_s((void __user *)arg, arg_size, ioctl_arg, arg_size);
                return (ret == 0) ? 0 : -1;
            }
        default:
            osal_log("cmd %x invalid\n", _IOC_DIR(cmd));
...
static int osal_platform_register(osal_dev *dev, struct dpal_platform_device *pfdev, struct dpal_platform_driver *pfdrv)
{
    ...
    if (ret != 0) {
        osal_log("dpal_plat_device_register failed! ret = %#x.\n", ret);
        return -1;
    }
    ...
    if (ret != 0) {
        dpal_plat_device_unregister(pfdev);
        osal_log("dpal_plat_driver_register failed! ret = %#x.\n", ret);
        return -1;
    }
    ...
    struct dpal_platform_driver *pfdrv)
{
    ...
    pfdrv->drv.name = dev->name;
    pfdrv->drv.pm = &g_osal_dev_pm_ops;
    (void)dpal_plat_driver_unregister(pfdrv);
    pfdev->name = dev->name;
    pfdev->dev.platform_data = (void*)dev;
    dpal_plat_device_unregister(pfdev);
    return 0;
