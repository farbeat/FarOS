 ...
 * Description:  os runtime monitor source.
 * Author:
 * Create: 2023-03-09
 */
...
int32_t g_wSysStatus = SYS_OS_READY;
...
typedef struct tagStatus {
    BOOL bNeedCheck;
    uint64_t uwRunTick;
    uint32_t uwTickThreshold;
    uint32_t uwBlockCount;
} runtime_status_cb_s;
...
void statuscheck_timer_callback(void)
{
    ...
    for (uwIndex = MON_TASK_BEGIN; uwIndex < MON_TASK_END; uwIndex++) {
        if ((g_statusCB[uwIndex].bNeedCheck) && (g_workCount & BIT(uwIndex))) {
...
{
    if (uwMailID < (uint32_t)MON_TASK_END) {
