...
#define INDEX_REG8              8
#define INDEX_REG9              9
#define INDEX_REG10             10
#define INDEX_REG11             11
#define INDEX_REG12             12
#define INDEX_REG13             13
#define INDEX_REG14             14
#define INDEX_REG15             15
...
#define read_custom_csr(csr) read_csr(csr)

#define write_custom_csr(csr, val) do {    \
    if (__builtin_constant_p(val))  {                 \
        asm volatile("li t0," "%0" : : "i"(val));     \
    } else {                                          \
        asm volatile("mv t0," "%0" : : "r"(val));     \
    }                                                 \
    asm volatile("csrw %0, t0" :: "i"(csr));      \
} while (0)

#define set_custom_csr(csr, bit) do {                  \
    if (__builtin_constant_p(bit) && (unsigned long)(bit) < 32) { \
        asm volatile("li t0," "%0" : : "i"(bit));             \
    } else {                                                  \
        asm volatile("mv t0," "%0" : : "r"(bit));             \
    }                                                         \
    asm volatile("csrs %0, t0" :: "i"(csr));              \
} while (0)

#define clear_custom_csr(csr, bit) do {                \
    if (__builtin_constant_p(bit) && (unsigned long)(bit) < 32) { \
        asm volatile("li t0," "%0" : : "i"(bit));             \
    } else {                                                  \
        asm volatile("mv t0," "%0" : : "r"(bit));             \
    }                                                         \
    asm volatile("csrc %0, t0" :: "i"(csr));            \
} while (0)
