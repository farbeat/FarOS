...
g_log_entry = [
    ...
    # BTH
    r"(dbg|info|err|warn)_log(\s*)\(",
    r"bts_(debug|info|error|warning)(_no_header)?(\s*)\(",
    r"bts_sample_log((_no_header)|(_with_func))?(\s*)\(",
    r"bts_(debug|info|err|warn)_log[0-4](\s*)\(",
    r"bts_dfx_print(_no_header)?(\s*)\(",
    r"bth_at_log_print_f(\s*)\(",
...
def get_log_level(str):
    m = re.search(r'LOG_LEVEL_ERROR|ERR_LOG|err_log|(GRAPHIC|DISP|LV)_LOGF|(GRAPHIC|DISP|LV)_LOGE|WEARABLE_LOGE|_ERR|bts_error|ERROR|FATAL', str)
    if m is not None:
        return 2,'ERROR'
    ...
    if m is not None:
        return 2,'ERROR'
    m = re.search(r'LOG_LEVEL_WARNING|WARN_LOG|warn_log|warning_log|(GRAPHIC|DISP|LV)_LOGW|WEARABLE_LOGW|_WARN|bts_warning', str)
    if m is not None:
        return 3,'WARNING'
    m = re.search(r'alg_ar_dbg|common_log_dbg|bts_debug', str)
    if m is not None:
        return 3,'WARNING'
    m = re.search(r'LOG_LEVEL_INFO|INFO_LOG|info_log|(GRAPHIC|DISP|LV)_LOGI|GRAPHIC_LOGP|WEARABLE_LOGI|_INFO|bts_info|bts_sample_log|bts_dfx_print|bth_at_log_print_f', str)
    if m is not None:
        return 5,'INFO'
    m = re.search(r'CHECK_STATE_SAME_RETURN', str)
    if m is not None:
        return 5,'INFO'
    m = re.search(r'LOG_LEVEL_DBG|(DBG|DEBUG)_LOG|(dbg|debug)_log|(GRAPHIC|DISP|LV)_LOGD|WEARABLE_LOGD|_DEBUG', str)
    if m is not None:
        return 6,'DBG'

    m = re.search(r'_error_log|_log_err|_ERROR_|(vau|dpu|jpeg|gmmu|vg)_err|ALOGE|MEDIA_(ERR|FATAL)_LOG|HMF_LOG(F|E)|MEDIA_HAL_LOGE|bts_error', str)
    if m is not None:
        return 2,'ERROR'
    m = re.search(r'_warning_log|_log_warn|_WARNING_|ALOGW|HMF_LOGW|MEDIA_HAL_LOGW|bts_warning', str)
    if m is not None:
        return 3,'WARNING'
    m = re.search(r'_info_log|_log_info|_INFO_|(vau|dpu|jpeg|gmmu|vg)_info|ALOGI|MEDIA_INFO_LOG|HMF_LOGI|MEDIA_HAL_LOGI|bts_info|bts_sample_log|bts_dfx_print|bth_at_log_print_f', str)
    if m is not None:
        return 5,'INFO'
    m = re.search(r'_debug_log|_log_debug|(vau|dpu|jpeg|gmmu|vg)_(dbg|print)|ALOG(V|D)|MEDIA_DEBUG_LOG|MEDIA_HAL_LOG(V|D)|bts_debug', str)
    if m is not None:
        return 6,'DBG'
