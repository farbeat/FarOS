...
class CMakeBuilder(BuildEnvironment):
    ...
    def build_target(self, target):
        ...
        if env.get('packet'):
            defines = env.get("defines")
            if "CFG_FLASH_SIZE=16" in defines:
    ...
    def pack_fwpkg(self, chip, target_name, flash_size):
        # bin packet all in one
        packet_script_path = os.path.join(pkg_tools_path, 'packet.py')
        print("packet ing...")
        errcode = exec_shell([self.python_path, packet_script_path, chip, target_name, flash_size], None, True)
        if errcode != 0:
            print("packet error!")
            self.print_build_result(target_name, errcode)
            sys.exit(1)
    ...
    def is_command_refresh(self, output_path):
        ...
        if not os.path.exists(cmd_path) or not os.path.exists(cache_path):
            return True
