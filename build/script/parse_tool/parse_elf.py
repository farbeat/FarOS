...
class ParseCtrl():
    ...
    def get_at_type(self,tag):#获取入口名
        ...
        if len(result) == 0:
            #print(tag.at_name, "|", tag.at_list['DW_AT_type'].val_attr)
            return None
        else:
    ...
    def parse_debug_info_lines(self,lines):
        pass_st_lines=['application.elf', 'wstp.elf','tstp.elf','Contents of the','Compilation Unit','Length:',
                        'Version:','Abbrev Offset:','Pointer Size:', 'Signature', 'Type Offset','$']
