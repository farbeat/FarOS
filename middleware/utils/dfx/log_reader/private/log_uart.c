    ...
    for (;;) {
        ...
        while (lr_ret == LOG_READER_RET_OK) {
            ...
            if ((lr_ret != LOG_READER_RET_OK) || ((lb_header.length - sizeof(lb_header)) != (l1 + l2))) {
                dfx_log_err("[LOG] error log header addr=0x%x, len= 0x%x\r\n",
                    (uintptr_t)(&lb_header), lb_header.length);
                dfx_log_err("[LOG] block1:addr=0x%x, len=0x%x\r\n", (uintptr_t)b1, l1);
                dfx_log_err("[LOG] block2:addr=0x%x, len=0x%x\r\n", (uintptr_t)b2, l2);
            ...
            if (wdg_left_time < LOG_MIN_WATCHDOG_LEFT_TIME && get_exception_status() != true) {
                uapi_watchdog_kick();
            }
        }
