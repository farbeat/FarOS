...
#if !defined(BUILD_RECOVERY_IMAGE) && !defined(BUILD_APPLICATION_SSB)
#include "cmsis_os2.h"
#endif
                                     ...
                                     cores_t src, uint32_t id)
{
...
#ifndef BUILD_APPLICATION_SSB
    uapi_diag_error_log(0, "cpu_utils_sys_reboot_req reason is 0x%x.", reboot_reason);
#endif
    ...
    reset_deal_with_fs();
#if defined(BUILD_APPLICATION_STANDARD) && (USE_COMPRESS_LOG_INSTEAD_OF_SDT_LOG == NO)
    PRINT("Reboot by core:%d with reason of 0x%x", src, reboot_reason);
#endif

#if (CORE == MASTER_BY_ALL)
    set_update_reset_cause_on_boot(false);
#endif
    cpu_utils_set_system_status_by_cause(reboot_reason);
#if !defined(BUILD_RECOVERY_IMAGE) && !defined(BUILD_APPLICATION_SSB)
    /* Give the semaphore to ensure the main logging task will be active */
    (void)osSemaphoreRelease(get_save_file_semaphore());
#else
    hal_reboot_chip();
#endif
