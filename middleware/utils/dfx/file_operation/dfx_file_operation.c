...
STATIC errcode_t dfx_file_node_info(const char *fullpath, dfx_file_report_node_hook handler,
    uintptr_t usr_data, const char *node_name)
{
    dfx_file_ls_node_info_t info;
...
errcode_t dfx_file_ls(const char *ls_path, dfx_file_report_node_hook handler, uintptr_t usr_data)
{
    ...
    do {
        ...
        if (pdirent != NULL) {
            ...
            dfx_file_node_info(fullpath, handler, usr_data, pdirent->d_name);
...
int32_t dfx_file_open_for_write(char *path)
{
    int fd;
    fd = open(path, O_RDWR | O_CREAT | O_TRUNC, 0);
    if (fd < 0) {
        if (dfx_file_mkdir(path) != 0) {
            dfx_log_err("[ERR]dfx_file_mkdir failed. \r\n");
...
errcode_t dfx_file_rmdir(const char *path)
{
    ...
    if (S_ISDIR(stat_info.st_mode)) {   /* 目录文件，递归删除目录中内容 */
        ...
        while ((dp = readdir(dirp)) != NULL) {
            ...
            /* 在内存资源紧张时，限制同时open的个数，因此需要先close，再递归，防止过度占用资源 */
            closedir(dirp);
            ret = dfx_file_rmdir((const char *)dir_name);   /* 递归删除 */
            if (ret != ERRCODE_SUCC) {
                dfx_free(0, dir_name);
                return ret;
            }
            /* 上一层递归完成后，重新open，保证下次循环的readdir正常进行 */
            dirp = opendir(path);
            dfx_free(0, dir_name);
        }
        closedir(dirp);
...
int32_t dfx_file_write(char *path, unsigned offset, const uint8_t *buf, uint32_t size)
{
    int fd;
    int ret;
    ssize_t len;
