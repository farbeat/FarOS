...
STATIC_UT errcode_t kv_upg_upgrade_key_helper(kv_store_t core, kv_key_header_t* key_header)
{
    ...
    if (key_header->valid == 0 && key_header->length == 0) {
        task_node.data.kv_erase.key = key_header->key_id;
        (void)kv_update_erase_key(core, &task_node);
    } else {
        attributes = 0;
        if (key_header->upgrade == 0) {
            attributes = NV_ATTRIBUTE_NON_UPGRADE;
        }
        /* 如果既是不可升级又是永久的，属性设置为永久即可 */
        if (key_header->type == 0) {
            attributes = NV_ATTRIBUTE_PERMANENT;
        }
...
STATIC_UT bool kv_upg_check_upgradeable(kv_key_header_t* key_header)
{
    ...
    (void)kv_store_get_key_attr(KV_STORE_APPLICATION, key_header->key_id, &key_length, &attr);
    if ((((uint32_t)attr & NV_ATTRIBUTE_PERMANENT) != 0) || (((uint32_t)attr & NV_ATTRIBUTE_NON_UPGRADE) != 0)) {
        return false;
    }
    return true;
}
...
errcode_t nv_upg_upgrade_task_process(void)
{
    ...
    /* 防止nv_init和nv升级不在一个线程时冲突 */
    while (!nv_direct_get_init_status()) {
        uapi_watchdog_kick();
        osDelay(KV_DELAY_MS);
    }
