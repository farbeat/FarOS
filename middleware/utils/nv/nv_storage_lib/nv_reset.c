...
STATIC errcode_t restore_retain_keys(void)
{
    ...
    while (cur_key != NULL && cur_key->kvalue != NULL) {
        ...
        (void)nv_direct_write_with_attr(cur_key->key, cur_key->kvalue, cur_key->kvalue_length, NULL, NULL);
...
STATIC errcode_t kv_backup_invalid_key(const kv_key_handle_t *key)
{
...
STATIC errcode_t kv_backup_dragpage_switch_unused_page(kv_page_handle_t dragpage, kv_page_handle_t unused_page)
{
    ...
    ret = kv_key_helper_copy_flash((uint32_t)(uintptr_t)&head_buffer,
        (uint32_t)dragpage.page_location, page_head_size);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
    ret = kv_key_write_flash((uint32_t)unused_page.page_location, page_head_size, (uint8_t *)&head_buffer);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
...
STATIC errcode_t kv_backup_dragpage_process(kv_page_handle_t dragpage)
{
    ...
    ret =  kv_backup_copy_unused_page_to_dragpage((uint32_t)dragpage.page_location,
        (uint32_t)unused_page.page_location);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
    /* 6、擦除工作区换页页 */
...
STATIC errcode_t kv_backup_find_write_position(uint16_t required_space, uint32_t *write_position)
{
    errcode_t ret;
    kv_page_handle_t page;
