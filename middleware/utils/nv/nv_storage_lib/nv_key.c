...
STATIC void kv_key_hash_crc_tag_start(kv_key_handle_t *key, uint32_t *crc_ret)
{
    *crc_ret = 0;
    if (key->header.enc_key != AES_KDFKEY_SDRK_TYPE) {
        return;
...
STATIC void kv_key_hash_crc_tag_update(kv_key_handle_t *key, uint8_t *data_chunk, uint32_t data_len, uint32_t *crc_ret)
{
...
STATIC void kv_key_hash_crc_tag_finish(kv_key_handle_t *key, uint8_t *calculated_hash, uint32_t crc_value,
    uint32_t crypto_handle)
{
...
STATIC errcode_t kv_key_hash_crc_tag_verify(kv_key_handle_t *key, uint8_t *read_hash_crc, uint8_t *cal_hash_crc,
