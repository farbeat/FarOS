...
{
    /* 使用了自定义的数据类型 注册kv_free函数 */
    CstlDupFreeFuncPair valueFunc = {NULL, nv_psram_free};
    g_hash_table = CstlHashCreate(NV_BACKET_SIZE, NULL, NULL, NULL, &valueFunc);
}
...
static bool nv_async_check_writeable(uint16_t key, nv_key_attr_t *attr, uint16_t kvalue_length)
{
    ...
    (attr == NULL || (attr != NULL && attr->encrypted == false)))
        || (attr == NULL && cur_key->non_upgrade == 1)) {
        return false;
    }
...
static errcode_t nv_async_msg_process(nv_queue_msg_handle_t *msg_data_t)
{
    ...
    attr.non_upgrade = (msg_data->non_upgrade == 1) ? true : false;
...
static errcode_t nv_async_thread_msg_event_init(void)
{
    ...
    osal_kthread_init nv_thread = {0};
    nv_thread.handler = (osal_kthread_handler)nv_async_msg_process_thread;
    nv_thread.data = NULL;
    nv_thread.stacksize = 0x1000;
    nv_thread.taskname = "nv_write_task";
    nv_thread.taskprio = NV_THREAD_PRIORITY_NUM;

    osal_task *task = osal_kthread_create_ext(&nv_thread);
    if (task == NULL) {
...
errcode_t nv_async_flush(void)
{
    ...
    CstlHashIterator it;
    errcode_t res;
    /* 遍历hash表，将当前还没刷新到flash上的NV刷新到flash */
    for (it = CstlHashIterBegin(g_hash_table); it != CstlHashIterEnd(g_hash_table);
        it = CstlHashIterNext(g_hash_table, it)) {
        nv_key_map_t *cur_data =  (nv_key_map_t *)CstlHashIterValue(g_hash_table, it);
        if (cur_data->flush_flag == false) {
            msg_data_t.key_id = cur_data->key_id;
            msg_data_t.key_ram_location = (uintptr_t)cur_data;
            res = nv_async_msg_process(&msg_data_t);
            if (res != ERRCODE_SUCC) {
                nv_log_err("[NV] nv_async_flush fail! key_id=0x%x", msg_data_t.key_id);
            }
        }
    }
