...
bool g_nv_init_is_done = false;

bool nv_direct_get_init_status(void)
{
    return g_nv_init_is_done;
}
...
errcode_t nv_direct_write_with_attr(uint16_t key_id, const uint8_t *kvalue, uint16_t kvalue_length, nv_key_attr_t *attr,
    nv_storage_completed_callback func)
{
    ...
    if (ret == ERRCODE_SUCC) {
        if ((((uint32_t)attribute & NV_ATTRIBUTE_PERMANENT) != 0) ||
            ...
            || ((((uint32_t)attribute & NV_ATTRIBUTE_NON_UPGRADE) != 0) && (attr == NULL))) {
...
errcode_t nv_direct_get_key_data(
    uint16_t key_id, uint16_t kvalue_max_length, uint16_t *kvalue_length, uint8_t *kvalue, nv_key_attr_t *attr)
{
