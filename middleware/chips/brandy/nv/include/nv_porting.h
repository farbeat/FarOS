...
#define nv_log_debug(fmt, args...) \
    do { \
        if (nv_get_debug_level() >= NV_LOG_LEVEL_DBG) { \
            uapi_diag_debug_log(0, fmt, ##args); \
        } \
    } while (0)

#define nv_log_info(fmt, args...) \
    do { \
        if (nv_get_debug_level() >= NV_LOG_LEVEL_INFO) { \
            uapi_diag_info_log(0, fmt, ##args); \
        } \
    } while (0)

#define nv_log_err(fmt, args...) \
    do { \
        if (nv_get_debug_level() >= NV_LOG_LEVEL_ERROR) { \
            uapi_diag_error_log(0, fmt, ##args); \
        } \
    } while (0)

#else /* #ifndef TEST_SUITE */

#define nv_log_err(fmt...)              PRINT(fmt)
#define nv_log_info(fmt...)             PRINT(fmt)
#define nv_log_debug(fmt...)

#endif /* #ifndef TEST_SUITE */
#else /* #ifdef KV_DEBUG */
#define nv_log_err(x...)
#define nv_log_info(x...)
#define nv_log_debug(x...)
#endif /* #ifdef KV_DEBUG */
