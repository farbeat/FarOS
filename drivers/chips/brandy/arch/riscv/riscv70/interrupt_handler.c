 ...
 * Description: RISCV70 Core interrupt handler
...
void nmi_default_handler(void)
{
    ...
    (void)set_csr(mie, MIP_NMIE);
    g_interrupt_running--;
}

void interrupt0_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE0);
    g_interrupt_running++;
    ...
    (void)set_csr(mie, MIP_LOCIE0);
    g_interrupt_running--;
}

void interrupt1_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE1);
    g_interrupt_running++;
    ...
    (void)set_csr(mie, MIP_LOCIE1);
    g_interrupt_running--;
}

void interrupt2_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE2);
    g_interrupt_running++;
    ...
    (void)set_csr(mie, MIP_LOCIE2);
    g_interrupt_running--;
}

void interrupt3_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE3);
    g_interrupt_running++;
    ...
    (void)set_csr(mie, MIP_LOCIE3);
    g_interrupt_running--;
}

void interrupt4_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE4);
    g_interrupt_running++;
    ...
    (void)set_csr(mie, MIP_LOCIE4);
    g_interrupt_running--;
}

void interrupt5_handler(void)
{
    (void)clear_csr(mie, MIP_LOCIE5);
    g_interrupt_running++;
