/******************************************************************************
 * @brief    RISCV70 vectors module implementation
 * Copyright (c) 2022 @CompanyNameMagicTag
******************************************************************************/
.equ ccause, 0xFC2
.equ MCAUSE_MASK_INT_NUM, 0xFFF
.equ NMI_EXCP_CODE, 0xFFF
.macro push_reg
    addi sp, sp, -32*4
...
trap_entry:
    ...
    la t0, MCAUSE_MASK_INT_NUM
    and  s3, t0, s3

    csrr a0, mscratch
    la t0, NMI_EXCP_CODE
    beq s3, t0, do_hard_fault_handler
    ...
    mret
1:
    tail do_hard_fault_handler

    .align 4
    .global trap_excp
    .section .text.trap_excp
    .option norvc
trap_excp:
    j trap_entry
...
trap_vector:
    ...
    .word default_local_interrupt_handler
    .word default_local_interrupt_handler
    .word default_local_interrupt_handler
    .word default_local_interrupt_handler
    .word default_local_interrupt_handler
