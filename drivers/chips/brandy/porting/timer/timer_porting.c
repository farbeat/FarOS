...
static int timer0_irq_handler(int i, const void *p)
{
    unused(i);
    unused(p);
...
static int timer1_irq_handler(int i, const void *p)
{
    unused(i);
    unused(p);
...
static int timer2_irq_handler(int i, const void *p)
{
    unused(i);
    unused(p);
...
uint32_t timer_porting_compensat_by_tcxo(uint64_t diff)
{
    if (diff < TIMER_COMPENSAT_1_CYCLE_BY_TCXO) {
        ...
        return (uint32_t)(diff / TIMER_COMPENSAT_1_CYCLE_BY_TCXO);
    }
}