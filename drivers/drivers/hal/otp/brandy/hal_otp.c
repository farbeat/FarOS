...
#define HAL_OTP_EN_REG_OFFSET  0
#define HAL_OTP_CLEAR_RESULT   0x0

#define HAL_OTP_REG_WIDTH       1U
...
uint32_t g_otp_region_read_address[] = { HAL_OTP0_READ_BASE_ADDR, HAL_OTP1_READ_BASE_ADDR, HAL_OTP2_READ_BASE_ADDR };
uint32_t g_otp_region_write_address[] = { HAL_OTP0_WRITE_BASE_ADDR,
                                          HAL_OTP1_WRITE_BASE_ADDR,
                                          HAL_OTP2_WRITE_BASE_ADDR };
uint32_t g_otp_region_ctl_address[] = { HAL_OTP0_CTRL, HAL_OTP1_CTRL, HAL_OTP2_CTRL };

void hal_otp_init(void)
{
    if (!hal_otp_refresh_read(HAL_OTP_REGION_1)) {
        return;
    }
}
...
{
    hal_otp_clear_all_write_regs(HAL_OTP_REGION_1);
}
...
{
    uint16_t byte_offset = byte_addr % OTP_REGION_MAX_BYTES;
    uint8_t region = (uint8_t)(byte_addr / OTP_REGION_MAX_BYTES);
...
static uint32_t hal_otp_get_write_addr(uint32_t byte_addr)
{
    ...
    // Each register has 16 bits, register address is (byte_offset / 2 * 4)
    return g_otp_region_write_address[region] + ((byte_offset >> 1U) << HAL_OTP_REG_SHIFT);
}
#endif
...
bool hal_otp_flush_write(hal_otp_region_e region)
{
    ...
    // power on efuse
    writew(HAL_OTP_POWER_ON_SWITCH, HAL_OTP_POWER_ON_VAL);
    uapi_tcxo_delay_us(HAL_OTP_POWER_ON_DELAY_US);

    // Set efuse to write mode
    writel(g_otp_region_ctl_address[region], HAL_OTP_WRITE_MODE);

    // Wait until the OTP_CTL clear
    while (regw(g_otp_region_ctl_address[region], HAL_OTP_CTL_REG_OFFSET) !=
           HAL_OTP_CLEAR_RESULT) {}

    // power off efuse
    writew(HAL_OTP_POWER_ON_SWITCH, HAL_OTP_POWER_OFF_VAL);
    hal_otp_clear_all_write_regs(region);
#else
    UNUSED(region);
#endif
    return true;
...
bool hal_otp_refresh_read(hal_otp_region_e region)
{
    ...
    writel(g_otp_region_ctl_address[region], HAL_OTP_READ_MODE);
    while (regw(g_otp_region_ctl_address[region], HAL_OTP_CTL_REG_OFFSET) !=
           HAL_OTP_CLEAR_RESULT) {}
#else
    UNUSED(region);
...
{
    uint32_t write_addr = g_otp_region_write_address[region];
    for (uint32_t i = 0; i < OTP_REGION_MAX_BYTES; i += HAL_OTP_REG_SHIFT) {
        *((volatile uint16_t *)((uintptr_t)(write_addr))) = 0;
        write_addr += HAL_OTP_REG_LENGTH;
    }
}

hal_otp_region_e hal_otp_get_region(uint32_t byte_addr)
{
    return (hal_otp_region_e)(byte_addr / OTP_REGION_MAX_BYTES);
}
...
bool hal_otp_write_buffer_operation(uint32_t address, const uint8_t *buffer, uint16_t length)
{
    for (uint32_t i = 0; i < length; i++) {
        if (!hal_otp_write_byte(address + i, buffer[i])) {
            hal_otp_region_e region_tmp = hal_otp_get_region(address + i);
            hal_otp_clear_all_write_regs(region_tmp);
            return false;
        }

        // The last byte of a region or the last byte all bytes need write.
        if ((((address + i + 1) % OTP_REGION_MAX_BYTES) == 0) || ((i + 1) == length)) {
            hal_otp_region_e region = hal_otp_get_region(address + i);
            if (!hal_otp_flush_write(region) || !hal_otp_refresh_read(region)) {
                hal_otp_clear_all_write_regs(region);
                return false;
            }
        }
