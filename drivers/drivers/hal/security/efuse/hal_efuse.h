 ...
 * @param  [in]  value        OTP byte value of the byte to be read
 * @retval ERRCODE_SUCC   Success.
 * @retval Other        Failure. For details, see @ref errcode_t
 * @else
 ...
 * @endif
 */
typedef errcode_t (*hal_efuse_get_die_id_t)(const uint8_t *buffer, uint16_t length);
 ...
 * @endif
 */
typedef errcode_t (*hal_efuse_get_chip_id_t)(const uint8_t *buffer, uint16_t length);
