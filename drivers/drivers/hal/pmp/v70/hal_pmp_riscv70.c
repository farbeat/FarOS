...
static errcode_t hal_pmp_riscv70_config(hal_pmp_conf_t *config)
{
    ...
    hal_pmp_riscv70_regs_set_pmpaddr(idx, addr);
    hal_pmp_riscv70_regs_set_memxattr(idx, attr);
    pmpx_config.rwx = config->cfg.rwx;
    pmpx_config.resv_0 = 0;
    pmpx_config.a = 0;
    pmpx_config.l = 0;
    hal_pmp_riscv70_regs_set_pmpxcfg(idx, pmpx_config);
...
static hal_pmp_funcs_t g_hal_pmp_riscv70_funcs = {
    .config = hal_pmp_riscv70_config
};

hal_pmp_funcs_t *hal_pmp_riscv70_funcs_get(void)
{
    return &g_hal_pmp_riscv70_funcs;
}