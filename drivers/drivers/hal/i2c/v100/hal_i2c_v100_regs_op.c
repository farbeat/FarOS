...
static volatile uint32_t *hal_i2c_v100_get_int_addr(i2c_bus_t bus, i2c_int_reg_t reg)
{
    volatile uint32_t *reg_addr = NULL;

    switch (reg) {
        case INTR_STAT:
            reg_addr = &g_i2c_regs[bus]->intr_stat;
            break;
        case INTR_MASK:
            reg_addr = &g_i2c_regs[bus]->intr_mask;
            break;
        default:
            reg_addr = &g_i2c_regs[bus]->raw_intr_stat;
            break;
    }
    return reg_addr;
}
...
{
    volatile uint32_t *reg_addr = NULL;
    reg_addr = hal_i2c_v100_get_int_addr(bus, reg);
...
{
    ic_intr_mask_data_t int_mask;
    int_mask.d32 = g_i2c_regs[bus]->intr_mask;
    int_mask.d32 = int_mask.d32 | (1U << (uint32_t)int_id);
    g_i2c_regs[bus]->intr_mask = int_mask.d32;
}
...
void hal_i2c_v100_set_transmit_data(i2c_bus_t bus, uint32_t cmd_type, uint32_t val)
{
    ...
    g_i2c_regs[bus]->data_cmd = data;
}

void hal_i2c_v100_set_gc_mode(i2c_bus_t bus)
{
    ic_tar_data_t data_cmd;

    data_cmd.d32 = g_i2c_regs[bus]->tar;
    data_cmd.b.gc_or_start = 0;
    data_cmd.b.special = 1;

    g_i2c_regs[bus]->tar = data_cmd.d32;
}
