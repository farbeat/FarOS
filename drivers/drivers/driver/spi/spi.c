...
static void spi_rx_isr(spi_bus_t bus)
{
    ...
    g_hal_funcs->ctrl(bus, SPI_CTRL_GET_ATTR, (uintptr_t)&attr);
    frame_bytes = hal_spi_frame_size_trans_to_frame_bytes(attr.frame_size);

    data.rx_bytes = frame_bytes;
    data.rx_buff = (uint8_t *)&rx_buff;
    g_hal_funcs->ctrl(bus, SPI_CTRL_CHECK_RX_FIFO_EMPTY, (uintptr_t)&rx_fifo_empty);
    /* Check that the SPI is opened */
    while (!rx_fifo_empty) {
        /* Read the data out of the SPI to clear the interrupt */
        /* Using a volatile variable to ensure the read always happens */
        g_hal_funcs->read(bus, &data, CONFIG_SPI_MAX_TIMEOUT);
        /* Only bother to try and record SPI data it there is an RX callback registered */
        ...
        for (uint32_t i = 0; i < frame_bytes; i++) {
            g_spi_rx_state[bus]->rx_buffer[g_spi_rx_state[bus]->new_rx_pos] = *(data.rx_buff + i);
            g_spi_rx_state[bus]->new_rx_pos++;
        }
        if (spi_rx_buffer_has_free_space(bus) == false) {
...
static errcode_t spi_register_rx_callback(spi_bus_t bus, const spi_xfer_data_t *data)
{
    ...
    errcode_t ret = g_hal_funcs->ctrl(bus, SPI_CTRL_EN_RXFI_INT, 1);
    spi_porting_unlock(bus, irq_sts);
...
#if defined(CONFIG_SPI_SUPPORT_MASTER) || defined(CONFIG_SPI_SUPPORT_SLAVE)
static errcode_t uapi_spi_param_check(spi_bus_t bus, spi_mode_t spi_mode, hal_spi_trans_mode_t trans_mode)
{
...
errcode_t uapi_spi_master_write(spi_bus_t bus, const spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    ret = uapi_spi_param_check(bus, SPI_MODE_MASTER, HAL_SPI_TRANS_MODE_RX);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
...
errcode_t uapi_spi_master_read(spi_bus_t bus, const spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    ret = uapi_spi_param_check(bus, SPI_MODE_MASTER, HAL_SPI_TRANS_MODE_TX);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
...
errcode_t uapi_spi_slave_write(spi_bus_t bus, const spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    ret = uapi_spi_param_check(bus, SPI_MODE_SLAVE, HAL_SPI_TRANS_MODE_RX);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
...
errcode_t uapi_spi_slave_read(spi_bus_t bus, const spi_xfer_data_t *data, uint32_t timeout)
{
    ...
    ret = uapi_spi_param_check(bus, SPI_MODE_SLAVE, HAL_SPI_TRANS_MODE_TX);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
