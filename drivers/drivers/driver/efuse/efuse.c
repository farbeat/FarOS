...
errcode_t uapi_efuse_write_bit(uint32_t byte_number, uint8_t bit_pos)
{
    ...
    uint8_t old_level = 0;
    pmu_vdd0p8_power_vget(&old_level);
    if (old_level != HAL_PMU_VDD0P8_1P0_0P925V) {
        pmu_vdd0p8_power_vset(HAL_PMU_VDD0P8_1P0_0P925V);
    }
    ...
    if (hal_funcs->write_op(byte_number, (uint8_t)bit(bit_pos), region) == ERRCODE_SUCC) {
        if (old_level != HAL_PMU_VDD0P8_1P0_0P925V) {
            pmu_vdd0p8_power_vset(old_level);
        }
        return ERRCODE_SUCC;
    }
...
errcode_t uapi_efuse_get_die_id(const uint8_t *buffer, uint16_t length)
{
...
errcode_t uapi_efuse_get_chip_id(const uint8_t *buffer, uint16_t length)
{
    if ((length == 0) || (buffer == NULL)) {
        return ERRCODE_FAIL;
    }
