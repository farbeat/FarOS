...
void otp_init(void)
{
    ...
    if (non_os_is_driver_initialised(DRIVER_INIT_OTP) == true) {
        return;
    }

    hal_otp_init();

    non_os_set_driver_initalised(DRIVER_INIT_OTP, true);
#endif
}
...
void otp_deinit(void)
{
    ...
    non_os_set_driver_initalised(DRIVER_INIT_OTP, false);
#endif
}
...
{
#if (OTP_HAS_WRITE_PERMISSION == YES)
    if (!non_os_is_driver_initialised(DRIVER_INIT_OTP)) {
