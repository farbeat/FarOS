...
static errcode_t i2c_ctrl_wait(i2c_bus_t bus, uint32_t ctrl_mask, uint64_t timeout)
{
    errcode_t ret;
    ...
    do {
        ...
        if (current_time > start_time && time_diff > timeout) {
...
static errcode_t i2c_wait(i2c_bus_t bus, i2c_wait_condition_t *condition, uint64_t timeout)
{
...
static errcode_t i2c_write_by_dma(i2c_bus_t bus, uint16_t dev_addr, hal_i2c_buffer_wrap_t *data)
{
    ...
    if (buffer_wrap.len != 0) {
        uapi_pm_add_sleep_veto(PM_ID_I2C_DMA);
        ret = i2c_transmit_dma(bus, &buffer_wrap);
        if (ret != ERRCODE_SUCC) {
            uapi_pm_remove_sleep_veto(PM_ID_I2C_DMA);
            return ret;
        }

        /* wait for trans */
        if (osal_sem_down_timeout(&(g_dma_trans[bus].dma_sem), I2C_DMA_TIMEOUT_MS) != OSAL_SUCCESS) {
            uapi_dma_end_transfer(g_dma_trans[bus].write_channel - 1);
            uapi_pm_remove_sleep_veto(PM_ID_I2C_DMA);
            g_dma_trans[bus].write_channel = 0;
...
static errcode_t i2c_read_do_send_cmd(i2c_bus_t bus, hal_i2c_buffer_wrap_t *data)
{
    ...
    if (buffer_wrap.len != 0) {
        ...
        if (osal_sem_down_timeout(&(g_dma_trans[bus].dma_sem), I2C_DMA_TIMEOUT_MS) != OSAL_SUCCESS) {
            uapi_dma_end_transfer(g_dma_trans[bus].write_channel - 1);
            uapi_dma_end_transfer(g_dma_trans[bus].read_channel - 1);
            g_dma_trans[bus].write_channel = 0;
            g_dma_trans[bus].read_channel = 0;
            return ERRCODE_I2C_DMA_TRANSFER_ERROR;
        }
...
static errcode_t i2c_write_int(i2c_bus_t bus, uint16_t dev_addr, hal_i2c_buffer_wrap_t *data_cfg)
{
    ...
    if (osal_sem_down(&g_i2c_int_sem[bus]) != OSAL_SUCCESS) {
        return ERRCODE_I2C_DMA_TRANSFER_ERROR;
    }
    return ret;
}
