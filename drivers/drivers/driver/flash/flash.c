...
{
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 0);
    hal_spi_v100_ser_set_ser(g_flash_config[id].bus, 1);
    hal_spi_v100_txftlr_set_tft(g_flash_config[id].bus, CONFIG_SPI_TX_FIFO_THRESHOLD);
    hal_spi_v100_rxftlr_set_rft(g_flash_config[id].bus, CONFIG_SPI_RX_FIFO_THRESHOLD);
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 1);
}
...
static bool flash_parameter_check(flash_id_t id, uint32_t addr, const uint8_t *dst, uint32_t length)
{
    ...
    if ((uint32_t)dst >= FLASH_START &&
        ((uint32_t)dst <= (FLASH_START + g_flash_device_parameter[g_flash_config[id].flash_manufacturer].flash_size))) {
        return false;
    }
...
static void trans_complete_wait_timeout(spi_bus_t bus, uint32_t timeout)
{
    ...
    do {
        ul_spi_trans_fifo_num = hal_spi_v100_txflr_get_txtfl(bus);
        ul_spi_state = hal_spi_v100_sr_get_busy(bus);
        ul_trans_timeout++;
...
static bool flash_write_cfg(flash_id_t id, uint32_t dest_addr)
{
    ...
    if (g_flash_config[id].mode == HAL_SPI_FRAME_FORMAT_QUAD) {
        if (mode_config->enter_xip_before_trans_type != HAL_SPI_TRANS_TYPE_INST_S_ADDR_Q) {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_PP_CMD);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, dest_addr);
        } else {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (FLASH_PP_CMD << FLASH_CMD_SHIFT_BIT) | dest_addr);
        }
    } else {
        hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (FLASH_PP_CMD << FLASH_CMD_SHIFT_BIT) | dest_addr);
    }
...
{
    while (hal_spi_v100_txflr_get_txtfl(g_flash_config[id].bus) >= SPI_TX_FIFO_DEPTH) {
    }
}
...
static void flash_write_word_by_once(flash_id_t id, uint32_t word, bool *send_flag)
{
    if (*send_flag) {
        ...
        hal_spi_v100_dr_set_dr(g_flash_config[id].bus, word);
        *send_flag = false;
    }
}
...
uint32_t uapi_flash_write_data(flash_id_t id, uint32_t addr, const uint8_t *src, uint32_t length)
{
    ...
    do {
        ...
        while (data_len > 0) {
            flash_wait_fifo_empty(id);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, flash_get_src_index(write_src));
...
uint32_t uapi_flash_write_data_without_align(flash_id_t id, uint32_t addr, const uint8_t *src, uint32_t length)
{
    ...
    do {
        ...
        while (data_len > 0) {
            flash_wait_fifo_empty(id);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, *write_src);
            data_len -= 1;
            if ((data_len > 0) || (write_len > 0)) {
                write_src += 1;
            }
...
static bool flash_read_cfg(flash_id_t id, uint32_t *one_process_len, uint32_t *length, uint32_t dest_addr)
{
    ...
    *length -= *one_process_len;
    ...
    if (g_flash_config[id].mode == HAL_SPI_FRAME_FORMAT_QUAD) {
        if (mode_config->enter_xip_after_enable_32bit_addr) {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_QRD_CMD);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, dest_addr);
        } else {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_QRD_CMD);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (dest_addr << SHIFT_WORD_15_8_BITS_TO_BYTE) |
                              g_flash_device_parameter[g_flash_config[id].flash_manufacturer].exit_xip);
        }
    } else {
        hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_RD_CMD << FLASH_CMD_SHIFT_BIT | dest_addr);
    }
...
static uint8_t flash_read_first_word(flash_id_t id, uint8_t *dst, uint8_t *first_cover_count)
{
    ...
    *((uint32_t *)first_read) = hal_spi_v100_dr_get_dr(g_flash_config[id].bus);
...
static uint8_t flash_read_last_word(flash_id_t id, uint8_t *dst, uint8_t *last_cover_count)
{
    ...
    *((uint32_t *)last_read) = hal_spi_v100_dr_get_dr(g_flash_config[id].bus);
    uint8_t *recv_data = dst;
    uint8_t count = 0;
                                       ...
                                       uint8_t *last_cover_count, bool left_len)
{
    ...
    // Check if the last 4 bytes need to be filled
        ...
        *((uint32_t *)dst) = hal_spi_v100_dr_get_dr(g_flash_config[id].bus);
...
uint32_t uapi_flash_read_data(flash_id_t id, uint32_t addr, uint8_t *dst, uint32_t length)
{
    ...
    do {
        ...
        while (one_process_len > 0) {
            ul_recv_num = hal_spi_v100_rxflr_get_rxtfl(g_flash_config[id].bus);
...
uint32_t uapi_flash_read_data_without_align(flash_id_t id, uint32_t addr, uint8_t *dst, uint32_t length)
{
    ...
    do {
        ...
        while (one_process_len > 0) {
            ul_recv_num = hal_spi_v100_rxflr_get_rxtfl(g_flash_config[id].bus);
            while ((ul_recv_num > 0) && (one_process_len > 0)) {
                *read_dst  = (uint8_t)hal_spi_v100_dr_get_dr(g_flash_config[id].bus);
                read_dst++;
                one_process_len--;
                ul_recv_num--;
                ul_recv_timeout = 0;
...
errcode_t uapi_flash_chip_erase(flash_id_t id)
{
    ...
    hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_CE_CMD);
...
errcode_t uapi_flash_block_erase(flash_id_t id, uint32_t addr, uint32_t length, bool is_wait)
{
    ...
    while (current_addr < end_addr) {
        ...
        if (g_flash_config[id].mode == HAL_SPI_FRAME_FORMAT_QUAD) {
            if (mode_config->enter_xip_before_trans_type != HAL_SPI_TRANS_TYPE_INST_S_ADDR_Q) {
                hal_spi_v100_dr_set_dr(g_flash_config[id].bus, cmd_id);
                hal_spi_v100_dr_set_dr(g_flash_config[id].bus, current_addr);
            } else {
                hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (cmd_id << FLASH_CMD_SHIFT_BIT) | current_addr);
            }
        } else {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (cmd_id << FLASH_CMD_SHIFT_BIT) | current_addr);
        }
...
errcode_t uapi_flash_sector_erase(flash_id_t id, uint32_t addr, bool is_wait)
{
    ...
    if (g_flash_config[id].mode == HAL_SPI_FRAME_FORMAT_QUAD) {
        if (mode_config->enter_xip_before_trans_type != HAL_SPI_TRANS_TYPE_INST_S_ADDR_Q) {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, FLASH_SE_CMD);
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, addr);
        } else {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (FLASH_SE_CMD << FLASH_CMD_SHIFT_BIT | addr));
        }
    } else {
        hal_spi_v100_dr_set_dr(g_flash_config[id].bus, (FLASH_SE_CMD << FLASH_CMD_SHIFT_BIT | addr));
    }
...
static errcode_t flash_excute_cmd(flash_id_t id, uint8_t cmd)
{
    ...
    hal_spi_v100_dr_set_dr(g_flash_config[id].bus, cmd);
...
static errcode_t flash_send_cmd_func(flash_id_t id, hal_spi_frame_format_t mode, uint8_t len, uint8_t *cmd)
{
    ...
    hal_spi_v100_ser_set_ser(g_flash_config[id].bus, 0);
    while (cmd_len > 0) {
        cmd_len--;
        if (send_cmd != NULL) {
            hal_spi_v100_dr_set_dr(g_flash_config[id].bus, *send_cmd++);
        }
    }
    hal_spi_v100_ser_set_ser(g_flash_config[id].bus, 1);
...
static void flash_switch_xip_spi_config(flash_id_t id)
{
    ...
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 0);
    hal_spi_v100_dmacr_set_rdmae(g_flash_config[id].bus, 1);
    hal_spi_v100_dmardlr_set_dmardl(g_flash_config[id].bus, 1);
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 1);
}
...
static void flash_exit_xip_spi_config(flash_id_t id)
{
    ...
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 0);
    hal_spi_v100_dmacr_set_rdmae(g_flash_config[id].bus, 0);
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 1);
}
...
{
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 0);
    hal_spi_v100_rx_sample_dly_set_rsd(g_flash_config[id].bus, 1);
    hal_spi_v100_ser_set_ser(g_flash_config[id].bus, 1);
    hal_spi_v100_txftlr_set_tft(g_flash_config[id].bus, CONFIG_SPI_TX_FIFO_THRESHOLD);
    hal_spi_v100_rxftlr_set_rft(g_flash_config[id].bus, CONFIG_SPI_RX_FIFO_THRESHOLD);
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 1);
...
void uapi_flash_set_spi_buad(flash_id_t id, uint32_t bus_clk, uint32_t freq_mhz)
{
    if (freq_mhz == 0) {
        return;
    }
    ...
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 0);
    hal_spi_v100_baudr_set_sckdv(g_flash_config[id].bus, clk_div);
    hal_spi_v100_ssienr_set_ssi_en(g_flash_config[id].bus, 1);
}
