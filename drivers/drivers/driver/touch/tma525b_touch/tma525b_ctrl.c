...
static uint8_t g_tp_send_buf[TMA525B_REG_WRITE_LEN] = {0};
static uint8_t g_tp_recv_buf[TMA525B_INPUT_SIZE] = {0};
...
ext_errno tp_i2c_data_read(uint16_t reg_addr, uint8_t *data_buf, uint32_t data_len)
{
    ...
    if (memset_s(g_tp_recv_buf, data_len, 0, data_len) != EOK) {
        tp_err("tp read buff set fail! ret=0x%x", ret);
        return EXT_ERR_FAILURE;
    }
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX0] = reg_addr & 0xff;
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX1] = (reg_addr >> OFFSET_8_BITS) & 0xff;
    ...
    if (ret != EOK) {
        tp_err("tp data buf content copy fail! ret=0x%x", ret);
        return ret;
    }
...
ext_errno tp_i2c_cmd_write(uint16_t cmd)
{
    ...
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX0] = cmd & 0xff;
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX1] = (cmd >> OFFSET_8_BITS) & 0xff;
    data.send_buf = g_tp_send_buf;
    data.send_len = TMA525B_CMD_SEND_LEN;
...
ext_errno tp_i2c_reg_write(uint16_t reg_addr, uint16_t reg_cfg)
{
    ...
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX0] = reg_addr & 0xff;
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX1] = (reg_addr >> OFFSET_8_BITS) & 0xff;
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX2] = reg_cfg & 0xff;
    g_tp_send_buf[TMA525B_I2C_SEND_INDEX3] = (reg_cfg >> OFFSET_8_BITS) & 0xff;
    data.send_buf = g_tp_send_buf;
