...
#if defined(TPTYPE_TMA525B)
#define TOUCH_I2C_BUS            TMA525B_I2C_BUS
#define TOUCH_INT_GPIO           TMA525B_INT_GPIO
#define TOUCH_RESET_GPIO         TMA525B_RESET_GPIO
#elif defined(TPTYPE_ZTW523)
#define TOUCH_I2C_BUS            ZTW523_I2C_BUS
#define TOUCH_INT_GPIO           ZTW523_INT_GPIO
#define TOUCH_RESET_GPIO         ZTW523_RESET_GPIO
#else
#define TOUCH_I2C_BUS            ZTW523_I2C_BUS
#define TOUCH_INT_GPIO           ZTW523_INT_GPIO
#define TOUCH_RESET_GPIO         ZTW523_RESET_GPIO
...
static void report_event_to_power_manager(input_event_info *touch_info)
{
    if (g_power_manager_cb != NULL) {
        g_power_manager_cb(touch_info->tp_event, 0);
    }
    return;
}
...
void touch_screen_suspend(void)
{
    touch_peripheral_api *touch_ptr = touch_screen_get_api();
    ext_errno ret = touch_ptr->touch_sleep();
    if (ret != EXT_ERR_SUCCESS) { tp_err("touch sleep ret=0x%x", ret); }
    i2c_save_reg(TOUCH_I2C_BUS);
    uapi_gpio_disable_interrupt(TOUCH_INT_GPIO); // close tp_int gpio interrupt
}

void touch_screen_resume(void)
{
    uapi_gpio_disable_interrupt(TOUCH_INT_GPIO); // close tp_int gpio interrupt
    i2c_recovery_reg(TOUCH_I2C_BUS);
#if defined(TPTYPE_TMA525B)
    tma525b_init();
    touch_peripheral_api *touch_ptr = (touch_peripheral_api*)touch_screen_get_api();
    ext_errno ret = touch_ptr->touch_resume();
    if (ret != EXT_ERR_SUCCESS) { tp_err("touch resume ret=0x%x", ret); }
#elif defined(TPTYPE_ZTW523)
    ztw523_resume();
#else
    ztw523_resume();
#endif
    uapi_gpio_enable_interrupt(TOUCH_INT_GPIO); // open tp_int gpio interrupt
}

void touch_suspend_for_i2c(void)
{
    i2c_save_reg(TOUCH_I2C_BUS);
}

void touch_resume_for_i2c(void)
{
    i2c_recovery_reg(TOUCH_I2C_BUS);
}

void touch_screen_restart(void)
{
    uapi_i2c_deinit(TOUCH_I2C_BUS);
    uapi_i2c_master_init(TOUCH_I2C_BUS, 400000, I2C_SPEED_MODE_FS); /* 400000: 400KHz */
#if defined(TPTYPE_TMA525B)
    tma525b_resume();
#elif defined(TPTYPE_ZTW523)
    ztw523_resume();
