...
static hal_tcxo_funcs_t *g_hal_funcs = NULL;
...
errcode_t uapi_tcxo_init(void)
{
    ...
    tcxo_port_register_hal_funcs();
    g_hal_funcs = hal_tcxo_get_funcs();
...
errcode_t uapi_tcxo_deinit(void)
{
    ...
    ret = g_hal_funcs->deinit();

    tcxo_port_unregister_hal_funcs();
