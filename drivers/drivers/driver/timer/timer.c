...
#define STANDARD_TIMER_MODE         1
#define HIGH_PRECISION_TIMER_MODE   2
#define HIGH_PRECISION_TIMER_LEVEL  500
...
#if defined(CONFIG_TIMER_SUPPORT_HIGH_PRECISION)
static uint8_t g_timer_useing_flag[TIMER_MAX_NUM];
#endif /* CONFIG_TIMER_SUPPORT_HIGH_PRECISION */
...
STATIC bool timer_update_timers_time(timer_index_t index, uint64_t update_amount_cycle)
{
    ...
    for (uint8_t i = 0; i < soft_time_num; i++) {
        if (g_timers_manager[index].timers[i].cycle <= update_amount_cycle) {
            ...
            g_timers_manager[index].timers[i].cycle -= update_amount_cycle;
        }
    }
...
STATIC uint32_t timer_get_next_timer(timer_index_t index)
{
    ...
    for (uint8_t i = 0; i < soft_time_num; i++) {
        if (g_timers_manager[index].timers[i].enable &&
            ...
            g_timers_manager[index].timers[i].is_run) {
            shortest_time_found = g_timers_manager[index].timers[i].cycle;
            timer_found = i;
        }
    }
    return timer_found;
...
{
    bool enable = true;
    uint64_t current_value = hal_timer_get_current_value(index, &enable);
    /* 当timer快到期的时候，又start了一个timer，在start过程中timer count会变成0 */
    if ((current_value != 0) || (enable == true)) {
...
#endif
        hal_timer_config_load(index, cycle_with_mask, 0);
        hal_timer_start(index);
...
errcode_t uapi_timer_init(void)
{
    ...
    memset_s(&g_timers_manager, sizeof(g_timers_manager), 0, sizeof(g_timers_manager));
...
#if defined(CONFIG_TIMER_SUPPORT_HIGH_PRECISION)
    if (g_timer_useing_flag[index] == HIGH_PRECISION_TIMER_MODE) {
        osal_irq_restore(irq_sts);
        return ERRCODE_TIMER_USING;
    }
#endif /* CONFIG_TIMER_SUPPORT_HIGH_PRECISION */
...
#if defined(CONFIG_TIMER_SUPPORT_HIGH_PRECISION)
    g_timer_useing_flag[index] = STANDARD_TIMER_MODE;
#endif /* CONFIG_TIMER_SUPPORT_HIGH_PRECISION */
    osal_irq_restore(irq_sts);
    return ERRCODE_SUCC;
}
...
{
    if ((timer == NULL) || (index >= TIMER_MAX_NUM)) {
        return ERRCODE_INVALID_PARAM;
    }
...
#if defined(CONFIG_TIMER_USING_OLD_VERSION)
    if (time_us < HIGH_PRECISION_TIMER_LEVEL) {
        return ERRCODE_INVALID_PARAM;
    }
#endif
    timer_info_t *timer_info = (timer_info_t *)timer;
...
{
    if ((index >= TIMER_MAX_NUM) || (irq_info == NULL) || (mode > TIMER_MODE_PERIODIC) || (time_us == 0) ||
        (time_us > uapi_timer_get_max_us()) || (callback == NULL)) {
        return ERRCODE_INVALID_PARAM;
    }

    if (g_timer_useing_flag[index] == STANDARD_TIMER_MODE) {
        return ERRCODE_TIMER_USING;
    }
...
#endif
    uint32_t irq_sts = osal_irq_lock();
    errcode_t ret = hal_timer_init(index, callback);
    if (ret != ERRCODE_SUCC) {
        osal_irq_restore(irq_sts);
        return ret;
    }
    timer_port_register_irq(index, irq_info->irq, irq_info->priority);

    uint64_t cycle = timer_porting_us_2_cycle(time_us);
    hal_timer_stop(index);
    hal_timer_config_load(index, cycle, mode);
    hal_timer_start(index);
    osal_irq_restore(irq_sts);
    g_timer_useing_flag[index] = HIGH_PRECISION_TIMER_MODE;
    return ERRCODE_SUCC;
}

errcode_t uapi_timer_reset_high_precision(timer_index_t index, timer_trigger_mode_t mode, uint32_t time_us)
{
    if ((index >= TIMER_MAX_NUM) || (mode > TIMER_MODE_PERIODIC) || (time_us == 0) ||
        (time_us > uapi_timer_get_max_us())) {
        return ERRCODE_INVALID_PARAM;
    }
    if (g_timer_useing_flag[index] != HIGH_PRECISION_TIMER_MODE) {
        return ERRCODE_TIMER_USING;
...
errcode_t uapi_timer_suspend(uintptr_t val)
{
    ...
    for (uint32_t index = 0; index < CONFIG_TIMER_MAX_NUM; index++) {
        if (g_timer_adapted[index] == false) {
            continue;
        }
        timer_update_timers_time(index,
            g_timers_manager[index].last_time_load_cycle - hal_timer_get_current_value(index, &enable));
        g_timers_manager[index].last_time_load_cycle = 0;
        timer_set_next_timer_interrupt(index);
    }
...
errcode_t uapi_timer_resume(uintptr_t val)
{
    ...
    for (uint32_t index = 0; index < CONFIG_TIMER_MAX_NUM; index++) {
        ...
        hal_timer_stop(index);
        hal_timer_config_load(index, timer_porting_us_2_cycle(1), 0);
        hal_timer_start(index);
    }
    osal_irq_restore(int_sts);
    return ERRCODE_SUCC;
