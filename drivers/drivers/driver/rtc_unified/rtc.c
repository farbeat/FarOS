...
STATIC bool rtc_update_rtcs_rtc(rtc_index_t index, uint64_t update_amount_cycle)
{
    ...
    for (uint32_t i = 0; i < soft_rtc_num; i++) {
        if (g_rtcs_manager[index].rtcs[i].is_run == false) {
            continue;
        }
...
{
    bool enable = true;
    uint64_t current_value = hal_rtc_get_current_value(index, &enable);
    /* 当rtc快到期的时候，又start了一个rtc，在start过程中rtc count会变成0 */
...
errcode_t uapi_rtc_init(void)
{
    ...
    memset_s(&g_rtcs_manager, sizeof(g_rtcs_manager), 0, sizeof(g_rtcs_manager));
...
errcode_t uapi_rtc_cycle_start(rtc_handle_t rtc, uint64_t cycle, rtc_callback_t callback, uintptr_t data)
{
    rtc_index_t index;
    uint64_t rtc_max_cycle = MAX_UINT64;
    if (unlikely(rtc == NULL) || unlikely(callback == NULL) ||
        unlikely(cycle == 0) || unlikely(cycle > rtc_max_cycle)) {
        return ERRCODE_INVALID_PARAM;
    }
    ...
    int32_t cycle_deviation = 0;
    index = rtc_info->index;
    load_cycle = rtc_get_cycle_with_mask(index, cycle, &cycle_deviation);
    rtc_info->cycle = load_cycle;
    rtc_info->callback = callback;
    rtc_info->data = data;

    uint64_t remain_rtc_cycle = rtc_get_rtc_till_interrupt(index);
    if (load_cycle <= remain_rtc_cycle) {
        if (remain_rtc_cycle != UINT64_MAX && g_rtcs_manager[index].last_rtc_load_cycle != 0) {
            rtc_update_rtcs_rtc(index, g_rtcs_manager[index].last_rtc_load_cycle - remain_rtc_cycle);
        }
        g_rtcs_manager[index].last_rtc_load_cycle = load_cycle;
        hal_rtc_stop(index);
        rtc_load_cycle_with_mask(index, cycle, cycle_deviation);
        hal_rtc_start(index);
    } else {
...
uint32_t uapi_rtc_get_latest_timeout(void)
{
    ...
    for (uint32_t index = 0; index < RTC_MAX_NUM; index++) {
        ...
        cur_count = hal_rtc_get_current_value(index, &enable);
    ...
    uint32_t nxt_us = rtc_porting_cycle_2_us(nxt_count);
    return nxt_us;
}
...
errcode_t uapi_rtc_suspend(uintptr_t val)
{
    ...
    for (uint32_t index = 0; index < RTC_MAX_NUM; index++) {
        ...
        rtc_update_rtcs_rtc(index,
            g_rtcs_manager[index].last_rtc_load_cycle - hal_rtc_get_current_value(index, &enable));
        g_rtcs_manager[index].last_rtc_load_cycle = 0;
        rtc_set_next_rtc_interrupt(index);
    }
