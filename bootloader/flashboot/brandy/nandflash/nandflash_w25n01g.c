...
#define W25N01G_GET_ECC_STATUS(x) ((0x30 & (x)) >> 4)
#define W25N01G_ECC_CORRECTION    0x00
#define W25N01G_ECC_1BIT_CORRET   0x01
#define W25N01G_ECC_4BIT_CORRET   0x02
#define W25N01G_ECC_ERROR         0x03
#define NAND_ID_DATA_SIZE         0x2

static uint32_t mc_w25n01g_read_page(uint32_t nand_page, uint8_t *data, uint32_t data_len,
                                     uint8_t *oob, uint32_t oob_len, const uint32_t *ecc_result)
{
    uint32_t ret;
    uint8_t status_reg;
    ...
    if (get_nand_flash()->enable_ecc == true) {
        if (W25N01G_GET_ECC_STATUS(status_reg) >= W25N01G_ECC_ERROR) {
            return MC_FAILURE;
        } else {
            return MC_SUCCESS;
        }
...
static uint32_t mc_w25n01g_init(void)
{
    nand_driver_init(SPEED_MID);
    PRINT("mc_w25n01g_init \r\n");
    uint32_t ret;
    ...
    if (get_nand_flash()->enable_ecc == true) {
        ret = mc_flash_write_sr(CMD_FLASH_SR2_ADDR, 0x18); /* set SR4 1, enable ecc */
    } else {
        ret = mc_flash_write_sr(CMD_FLASH_SR2_ADDR, 0x08); /* set SR3 1, enable buffer */
    }
    uint8_t status_reg;
...
static uint16_t mc_w25n01g_check_id(void)
{
    qspi_data_type_def g_qspi_data = {0};
...
struct nand_driver_struct g_w25n01g_driver = {
    mc_w25n01g_init,
    mc_w25n01g_check_id,
    mc_w25n01g_read_page,
    mc_flash_write_page,
    mc_flash_erase_block,
    mc_flash_mark_bad_block,
    mc_flash_check_bad_block,
    W25N01G_BYTES_PER_PAGE,
    W25N01G_BYTES_PER_OOB,
    W25N01G_PAGES_PER_BLOCK,
    W25N01G_TOTAL_BLOCKS,
    W25N01G_CHIP_ID,
    W25N01G_OOB_TOTAL_LEN,
    W25N01G_OOB_PER_SIZE,
    W25N01G_OOB_PER_SIZE,
    W25N01G_OOB_PER_SIZE,
    W25N01G_OOB_PER_SIZE,
    W25N01G_OOB_1_OFFSET,
    W25N01G_OOB_2_OFFSET,
    W25N01G_OOB_3_OFFSET,
    W25N01G_OOB_4_OFFSET,
#ifdef ENABLE_ECC
    true
#else
    false
