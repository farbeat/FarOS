...
#endif
int32_t sdio_register_card(struct rt_mmcsd_card *card);
...
rt_inline int32_t sdio_match_card(struct rt_mmcsd_card *card, const struct rt_sdio_device_id *id);
void sdio_free_cis(struct rt_sdio_function *func);

#define LOS_TASK_STATUS_DETACHED                0x0100    // add liteos type and function in mmc code

int32_t sdio_io_send_op_cond(struct rt_mmcsd_host *host, uint32_t ocr, uint32_t *cmd5_resp)
{
                          ...
                          uint8_t               raw)
{
    ...
    if (reg_addr & ~SDIO_ARG_CMD53_REG_MASK) { return -RT_ERROR; }
    ...
    if (!controller_is_spi(card->host)) {
        if (cmd.resp[0] & R5_ERROR) { return -RT_EIO; }
        if (cmd.resp[0] & R5_FUNCTION_NUMBER) { return -RT_ERROR; }
        if (cmd.resp[0] & R5_OUT_OF_RANGE) { return -RT_ERROR; }
    }

    if (!rw || raw) {
        if (controller_is_spi(card->host)) {
            *pdata = (cmd.resp[0] >> 8) & 0xFF; // 8
        } else {
                            ...
                            uint32_t             blksize)
{
    ...
    RT_ASSERT(card != RT_NULL);
    RT_ASSERT(fn <= SDIO_MAX_FUNCTIONS);
    RT_ASSERT(blocks != 1 || blksize <= RT_SDIO_STACK_SIZE);
    RT_ASSERT(blocks != 0);
    RT_ASSERT(blksize != 0);

    if (addr & ~SDIO_ARG_CMD53_REG_MASK) { return -RT_ERROR; }
    ...
    if (blocks == 1 && blksize <= RT_SDIO_STACK_SIZE) {
        cmd.arg |= (blksize == RT_SDIO_STACK_SIZE) ? 0 : blksize;      /* 512 byte mode */
    } else {
...
rt_inline uint32_t sdio_max_block_size(struct rt_sdio_function *func)
{
    ...
    return MIN(size, 512u); /* maximum size for byte mode 512 */
}
                                  ...
                                  uint32_t                  len)
{
    ...
    if (func->card->cccr.multi_block && (len > sdio_max_block_size(func))) {
        ...
        max_blks = MIN(max_blks, 511u); // 511
...
uint8_t sdio_io_readb(struct rt_sdio_function *func, uint32_t reg, int32_t *err)
{
    uint8_t data = 0;
    int32_t ret;
...
int32_t sdio_io_writeb(struct rt_sdio_function *func, uint32_t reg, uint8_t data)
{
    return sdio_io_rw_direct(func->card, 1, func->num, reg, &data, 0);
}

rt_uint16_t sdio_io_readw(struct rt_sdio_function *func, uint32_t addr, int32_t *err)
{
    int32_t ret;
    uint32_t dmabuf;
    ...
    ret = sdio_io_rw_extended_block(func, 0, addr, 1, (uint8_t *)&dmabuf, 2); // 2
    if (ret) {
        if (err) {
            *err = ret;
        }
...
int32_t sdio_io_writew(struct rt_sdio_function *func, rt_uint16_t data, uint32_t addr)
{
    uint32_t dmabuf = data;
    return sdio_io_rw_extended_block(func, 1, addr, 1, (uint8_t *)&dmabuf, 2); // 2
}

uint32_t sdio_io_readl(struct rt_sdio_function *func, uint32_t addr, int32_t *err)
{
    int32_t ret;
    uint32_t dmabuf;
    ...
    ret = sdio_io_rw_extended_block(func, 0, addr, 1, (uint8_t *)&dmabuf, 4); // 4
    if (ret) {
        if (err) {
            *err = ret;
        }
...
int32_t sdio_io_writel(struct rt_sdio_function *func,  uint32_t data, uint32_t addr)
{
    uint32_t dmabuf = data;
    return sdio_io_rw_extended_block(func, 1, addr, 1, (uint8_t *)&dmabuf, 4); // 4
}

int32_t sdio_io_read_multi_fifo_b(struct rt_sdio_function *func, uint32_t addr, uint8_t *buf, uint32_t len)
{
    return sdio_io_rw_extended_block(func, 0, addr, 0, buf, len);
}

int32_t sdio_io_write_multi_fifo_b(struct rt_sdio_function *func, uint32_t addr, uint8_t *buf, uint32_t len)
{
    return sdio_io_rw_extended_block(func, 1, addr, 0, buf, len);
}

int32_t sdio_io_read_multi_incr_b(struct rt_sdio_function *func, uint32_t addr, uint8_t *buf, uint32_t len)
{
    return sdio_io_rw_extended_block(func, 0, addr, 1, buf, len);
}

int32_t sdio_io_write_multi_incr_b(struct rt_sdio_function *func, uint32_t addr, uint8_t *buf, uint32_t len)
{
...
static int32_t sdio_read_cccr(struct rt_mmcsd_card *card)
{
    ...
    card->cccr.sdio_version = (data & 0xf0) >> 4; // 4
...
static int32_t cistpl_funce_func0(struct rt_mmcsd_card *card, const uint8_t *buf, uint32_t size)
{
    ...
    /* TPLFE_FN0_BLK_SIZE */
    card->cis.func0_blk_size = buf[1] | (buf[2] << 8); // 2 8

    /* TPLFE_MAX_TRAN_SPEED */
    card->cis.max_tran_speed = speed_value[(buf[3] >> 3) & 15] * speed_unit[buf[3] & 7]; // 3 15 7
    return 0;
}

static int32_t cistpl_funce_func(struct rt_sdio_function *func, const uint8_t *buf, uint32_t size)
{
    uint32_t version;
    uint32_t min_size;

    version = func->card->cccr.sdio_version;
    min_size = (version == SDIO_SDIO_REV_1_00) ? 28 : 42; // 28 42
    ...
    /* TPLFE_MAX_BLK_SIZE */
    func->max_blk_size = buf[12] | (buf[13] << 8); // 12 13 8

    /* TPLFE_ENABLE_TIMEOUT_VAL, present in ver 1.1 and above */
    if (version > SDIO_SDIO_REV_1_00) {
        func->enable_timeout_val = (buf[28] | (buf[29] << 8)) * 10; // 28 29 8 10
    } else {
        ...
        switch (tpl_code) {
            case CISTPL_MANFID:
                ...
                if (func->num != 0) {
                    ...
                    func->manufacturer |= curr->data[1] << 8; // 8
                    func->product = curr->data[2]; // 2
                    func->product |= curr->data[3] << 8; // 8 3
                } else {
                    card->cis.manufacturer = curr->data[0];
                    card->cis.manufacturer |= curr->data[1] << 8; // 8
                    card->cis.product = curr->data[2]; // 2
                    card->cis.product |= curr->data[3] << 8; // 3 8
                }
                break;
            case CISTPL_FUNCE:
                if (func->num != 0) {
            ...
            default:
                ...
                *prev = curr;
                prev = &curr->next;
                LOG_D("function %d, CIS tuple code %#x, length %d", func->num, tpl_code, tpl_link);
                break;
        }
...
static int32_t sdio_initialize_function(struct rt_mmcsd_card *card, uint32_t func_num)
{
    int32_t ret;
...
static int32_t sdio_set_highspeed(struct rt_mmcsd_card *card)
{
    ...
    if (!(card->host->flags & MMCSD_SUP_HIGHSPEED)) { return 0; }
...
static int32_t sdio_set_bus_wide(struct rt_mmcsd_card *card)
{
    ...
    if (!(card->host->flags & MMCSD_BUSWIDTH_4)) { return 0; }
...
static int32_t sdio_init_card(struct rt_mmcsd_host *host, uint32_t ocr)
{
    ...
    function_num = (cmd5_resp & 0x70000000) >> 28; // 28
    ...
    if (card->flags & CARD_FLAG_HIGHSPEED) {
        mmcsd_set_clock(host, 50000000); // 50000000
    } else {
...
int32_t deinit_sdio_free_mem(struct rt_mmcsd_host *host)
{
    if (host == RT_NULL || host->card == RT_NULL) {
        return -RT_EINVAL;
    }
    for (int i = 0; i < host->card->sdio_function_num + 1; i++) {
        if (host->card->sdio_function[i]) {
            sdio_free_cis(host->card->sdio_function[i]);
            rt_free(host->card->sdio_function[i]);
            host->card->sdio_function[i] = RT_NULL;
...
static void sdio_irq_thread(void *param)
{
    ...
    while (1) {
        if (hi_mmc_sem_pend(host->sdio_irq_sem, MMC_SEM_WAIT_FOREVER) == RT_EOK) {
...
int32_t sdio_attach_irq(struct rt_sdio_function *func, rt_sdio_irq_handler_t *handler)
{
    int32_t ret;
    uint8_t reg;
...
void sdio_irq_wakeup(struct rt_mmcsd_host *host)
{
    ...
    if (host->sdio_irq_sem != 0) {
        hi_mmc_sem_post(host->sdio_irq_sem);
    }
}
...
int32_t sdio_enable_func(struct rt_sdio_function *func)
{
    ...
    timeout = uapi_systick_get_count() + func->enable_timeout_val * RT_TICK_PER_SECOND / 1000; // 1000
    while (1) {
        reg = sdio_io_readb(func0, SDIO_REG_CCCR_IO_RDY, &ret);
        if (ret) { goto err; }
        if (reg & (1 << func->num)) { break; }
        ret = -RT_ETIMEOUT;
        if (uapi_systick_get_count() > timeout) { goto err; }
    }
...
int32_t sdio_set_block_size(struct rt_sdio_function *func, uint32_t blksize)
{
    ...
    if (blksize == 0) {
        ...
        blksize = MIN(blksize, 512u); // 512
    }

    ret = sdio_io_writeb(func0, SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_BLKSIZE, blksize & 0xff);
    if (ret) { return ret; }

    ret = sdio_io_writeb(func0, SDIO_REG_FBR_BASE(func->num) + SDIO_REG_FBR_BLKSIZE + 1, (blksize >> 8) & 0xff); // 8
    if (ret) { return ret; }
