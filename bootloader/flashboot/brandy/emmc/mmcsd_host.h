...
#define MMCSD_SUP_HS200     (1 << 6) /* support hs200 */
#define MMCSD_SUP_HS400     (1 << 7) /* support hs400 */
#define MMCSD_NON_OS_ENABLE (1 << 8) /* support non os enum */
#define MMCSD_BUSWIDTH_1    (1 << 9) /* support 1 bit width */
#define MMCSD_NO_USE_MUTEX  (1 << 10) /* no use mutex */
    ...
    mmc_sem sdio_irq_sem;
    mmc_thread sdio_irq_thread;
    uint8_t id;
    uint32_t driver_cap; /* phy mode driver capability */
    void *private_data;
};
