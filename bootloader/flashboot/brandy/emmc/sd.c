...
rt_inline uint32_t GET_BITS(uint32_t *resp, uint32_t start, uint32_t  size)
{
    ...
    const int32_t __off = 3 - ((start) / 32); // 3 32
...
static int32_t mmcsd_parse_csd(struct rt_mmcsd_card *card)
{
    ...
    switch (csd->csd_structure) {
        case 0:
            ...
            card->card_capacity = (csd->c_size + 1) << (csd->c_size_mult + 2); // 2
            card->card_capacity *= card->card_blksize;
            card->card_capacity >>= 10; /* unit:10KB */
        ...
        case 1:
            ...
            csd->rd_blk_misalign = GET_BITS(resp, 77, 1); // 77 1
            csd->dsr_imp = GET_BITS(resp, 76, 1); // 76 1
            csd->c_size = GET_BITS(resp, 48, 22); // 48 22
            ...
            card->card_blksize = 512; // 512
            card->card_capacity = (csd->c_size + 1) * 512;  /* unit:512KB */
            card->tacc_clks = 0;
            card->tacc_ns = 0;
            card->max_data_rate = tran_unit[csd->tran_speed&0x07] * tran_value[(csd->tran_speed&0x78)>>3]; // 3
            break;
...
static int32_t mmcsd_parse_scr(struct rt_mmcsd_card *card)
{
    ...
    resp[3] = card->resp_scr[1]; // 3 1
    resp[2] = card->resp_scr[0]; // 2 0
    scr->sd_version = GET_BITS(resp, 56, 4); // 56 4
    scr->sd_bus_widths = GET_BITS(resp, 48, 4); // 48 4
...
static int32_t mmcsd_switch(struct rt_mmcsd_card *card)
{
    ...
    if (buf[13] & 0x02) { // 13
        card->hs_max_data_rate = 50000000; // 50000000
    }
...
rt_err_t mmcsd_app_set_bus_width(struct rt_mmcsd_card *card, int32_t width)
{
    ...
    err = mmcsd_send_app_cmd(card->host, card, &cmd, 3); // 3 times
    if (err) {
        return err;
    }
...
rt_err_t mmcsd_send_app_op_cond(struct rt_mmcsd_host *host, uint32_t ocr, uint32_t *rocr)
{
    ...
    if (controller_is_spi(host)) {
        cmd.arg = ocr & (1 << 30); /* SPI only defines one bit 30 */
    } else {
        cmd.arg = ocr;
    }
    ...
    for (i = 100; i; i--) { // 100
        err = mmcsd_send_app_cmd(host, RT_NULL, &cmd, 3); // 3
        if (err) {
            break;
        }
...
rt_err_t mmcsd_send_if_cond(struct rt_mmcsd_host *host, uint32_t ocr)
{
    ...
    cmd.arg = (((ocr & 0xFF8000) != 0) << 8) | 0xAA; // 8
...
static int32_t mmcsd_sd_init_card(struct rt_mmcsd_host *host, uint32_t ocr)
{
    struct rt_mmcsd_card *card;
    int32_t err;
    uint32_t resp[4]; // 4
...
int32_t init_sd(struct rt_mmcsd_host *host, uint32_t ocr)
{
    ...
    if (ocr & VDD_165_195) {
        LOG_I(" SD card claims to support the " "incompletely defined 'low voltage range'. This " "will be ignored.");
        ocr &= ~VDD_165_195;
    }
