...
extern "C" {
#endif
#define rt_memset(s, c, count)      memset_s(s, count, c, count)
#define rt_memcpy(dst, src, count)  memcpy_s(dst, count, src, count)

#ifdef MMC_OPEN_ALL_LOG
#define mmcsd_dbg(fmt, ...)  print_str(fmt, ##__VA_ARGS__)
#else
...
#define DATA_CMD_STOP   (0x1<<5)
#define DATA_CMD_STOP_ENABLE(data)   ((data)->flags & DATA_CMD_STOP)
...
struct rt_mmcsd_erase {
    uint32_t start_addr;
    uint32_t len;
    uint32_t type;
#define ERASE_TYPE_ERASE_GROUP      0x0
#define ERASE_TYPE_TRIM_SECTOR      0x1
#define ERASE_TYPE_DISCARD_SECTOR   0x3
};
struct rt_mmcsd_req {
    struct rt_mmcsd_data  *data;
    struct rt_mmcsd_cmd   *cmd;
    struct rt_mmcsd_cmd   *stop;
    struct rt_mmcsd_erase *erase;
};
...
rt_inline uint32_t __rt_fls(uint32_t val)
{
    ...
    if (!(val & 0xc0000000u)) {
        val <<= 2; // 2bootloader/flashboot/brandy/emmc/sd.c
        bit -= 2; // 2
    }
    if (!(val & 0x80000000u)) {
        bit -= 1;
...
void hi_mmc_mutex_init(mmc_mutex *mux);
void mmcsd_host_lock(struct rt_mmcsd_host *host);
void mmcsd_host_unlock(struct rt_mmcsd_host *host);
void hi_mmc_mutex_del(mmc_mutex mux);
int hi_mmc_sem_init(mmc_sem *sem);
int hi_mmc_sem_post(mmc_sem sem);
int hi_mmc_sem_pend(mmc_sem sem, uint32_t timeout);
int hi_mmc_sem_del(mmc_sem sem);
