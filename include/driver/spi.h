...
/**
 * @if Eng
 * @brief  Write data from host to slave.
 *         There are three transfer modes, but cannot be used in the same bus at the same time:
 *         - Polling mode
 *         - DMA mode
 *         - Interrupt mode
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t.
 ...
 * @retval Other        Failure. For details, see @ref errcode_t.
 * @else
 * @brief  将数据从主机写入到从机。一共有以下三种传输模式，但是不能在同一bus中同时使用。
 *         - 轮询模式
 *         - DMA模式
 *         - 中断模式
 * @param  [in]  bus 指定的SPI接口，参考 @ref spi_bus_t 。
 ...
 *         - Polling mode
 *         - DMA mode
 *         - Interrupt mode(Register callback in there. Data reading is implemented in the callback function.)
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t.
 ...
 * @retval Other        Failure. For details, see @ref errcode_t.
 * @else
 * @brief  从slave读取数据。一共有以下三种传输模式，但是不能在同一bus中同时使用。
 *         - 轮询模式
 *         - DMA模式
 *         - 中断模式(此处仅注册回调，数据读取在回调函数中实现)
 * @param  [in]  bus 指定的SPI接口，参考 @ref spi_bus_t 。
...
/**
 * @if Eng
 * @brief  Write and read data in host in polling mode.
 * @param  [in]  bus The SPI bus. For details, see @ref spi_bus_t.
...
/**
 * @if Eng
 * @brief  Write data from slave to host.
 *         There are three transfer modes, but cannot be used in the same bus at the same time:
 *         - Polling mode
 *         - DMA mode
 *         - Interrupt mode
 ...
 * @retval Other        Failure. For details, see @ref errcode_t.
 * @else
 * @brief  从机向主机写入数据。一共有以下三种传输模式，但是不能在同一bus中同时使用。
 *         - 轮询模式
 *         - DMA模式
 *         - 中断模式
 * @param  [in]  bus 指定的SPI接口，参考 @ref spi_bus_t 。
 ...
 * @retval Other        Failure. For details, see @ref errcode_t.
 * @else
 * @brief  从主机读取数据。一共有以下三种传输模式，但是不能在同一bus中同时使用。
 *         - 轮询模式
 *         - DMA模式
 *         - 中断模式(此处仅注册回调，数据读取在回调函数中实现)
 * @param  [in]  bus 指定的SPI接口，参考 @ref spi_bus_t 。
